...
export default async function HomePage() {
  ...
  return (
    <main className="min-h-screen flex flex-col">
      {/* Hero */}
      <section className="bg-ink px-4 pt-10 pb-8 border-b border-border">
      ...
      <section className="bg-surface border-b border-border px-4 py-4">
        <div className="max-w-lg mx-auto flex gap-4">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="text-cream font-medium">{stats.totalVoters.toLocaleString()}</span>
            <span className="text-muted">votantes</span>
          </div>
          <div className="w-px bg-border" />
          <div className="flex items-center gap-2 text-sm">
            <Car size={16} className="text-rust shrink-0" />
            <span className="text-cream font-medium">{stats.totalNominations.toLocaleString()}</span>
            <span className="text-muted">clasificados</span>
          </div>
          <div className="w-px bg-border" />
          <div className="flex items-center gap-2 text-sm">
            <Trophy size={16} className="text-gold shrink-0" />
            <span className="text-cream font-medium">{stats.maxQualifiers}</span>
      ...
      <section className="px-4 py-6 max-w-lg mx-auto w-full space-y-3">
        {stats.nominationsOpen && (
          <Link
            href="/nominaciones"
            className="flex items-center justify-between w-full bg-rust text-ink rounded-2xl px-5 py-4 shadow-lg shadow-rust/20 hover:bg-rust-dark active:scale-[0.98] transition-all"
          >
            <div>
              <p className="font-display text-2xl tracking-wide">CLASIFICAR AL MUNDIAL</p>
              <p className="text-ink/70 text-xs mt-0.5">Elegí los autos más clavos · hasta 5 por persona</p>
            </div>
        ...
        <Link
          href="/fixture"
          className="flex items-center justify-between w-full bg-surface border-2 border-border rounded-2xl px-5 py-4 hover:border-rust/40 active:scale-[0.98] transition-all"
        >
          <div>
            <p className="font-display text-2xl text-cream tracking-wide">VER FIXTURE</p>
      ...
      {topCars.length > 0 && (
        <section className="px-4 pb-8 max-w-lg mx-auto w-full">
          <h2 className="font-display text-2xl text-cream mb-4 tracking-wide">
            CLASIFICADOS HASTA AHORA
          </h2>
          <div className="space-y-2">
            {topCars.map((car, i) => (
              <div key={car.car_name} className="flex items-center gap-3 bg-surface rounded-xl px-4 py-3 border border-border">
                <span className={`font-display text-xl w-7 text-center shrink-0 ${
                  i === 0 ? "text-gold" : i === 1 ? "text-muted" : i === 2 ? "text-rust/70" : "text-muted/40"
                }`}>
                  {i + 1}
                </span>
                <span className="flex-1 text-sm text-cream font-medium min-w-0 truncate">{car.car_name}</span>
      ...
      <footer className="mt-auto px-4 py-6 text-center border-t border-border">
        <p className="text-xs text-muted">
          ...
          <a href="https://x.com/emipanelli" target="_blank" rel="noopener noreferrer" className="text-rust">
            @emipanelli
          </a>
        </p>
      </footer>
