...
export default function CarSearch({ index, value, onChange, onRemove, existingCars, canRemove }: CarSearchProps) {
  ...
  const handleSelect = (car: string) => { setQuery(car); onChange(car); setOpen(false); };
  const handleClear = () => { setQuery(""); onChange(""); setOpen(false); inputRef.current?.focus(); };
  const isConfirmed = !!value;

  return (
    <div ref={containerRef} className="relative">
      <div className={`flex items-center gap-2 rounded-xl border-2 px-3 py-3 transition-all bg-white ${
        isConfirmed ? "border-rust/40 bg-rust/5" : open ? "border-rust/30" : "border-border"
      }`}>
      ...
      {open && (
        <div className="absolute z-50 top-full mt-1 left-0 right-0 bg-white border-2 border-border rounded-xl shadow-xl overflow-hidden max-h-60 overflow-y-auto">
          {suggestions.length > 0 ? (
            <>
              {suggestions.map((car) => (
                <button key={car} type="button"
                  onMouseDown={(e) => e.preventDefault()}
                  onClick={() => handleSelect(car)}
                  className="w-full text-left px-4 py-3 text-sm text-ink hover:bg-surface active:bg-rust/5 border-b border-border/50 last:border-0 transition-colors"
                >
                  {car}
                </button>
              ))}
