...
interface GroupData { letter: string; cars: (TournamentCar | null)[]; }

function buildGroups(cars: TournamentCar[], maxQualifiers: number): GroupData[] {
...
function GroupCard({ group, phase }: { group: GroupData; phase: TournamentPhase }) {
  ...
  return (
    <div className="bg-surface rounded-2xl border border-border overflow-hidden">
      <div className="bg-ink px-4 py-2.5 flex items-center justify-between border-b border-border">
        <span className="font-display text-cream text-2xl tracking-wider">GRUPO {group.letter}</span>
        {isEmpty && (
          <span className="text-xs text-muted font-medium uppercase tracking-wider">Clasificando...</span>
        )}
      </div>
      <div className="divide-y divide-border">
        {group.cars.map((car, i) => (
          <div key={i} className="flex items-center gap-3 px-4 py-3 min-h-[52px]">
            ...
            {car ? (
              <span className="text-sm text-cream font-medium">{car.car_name}</span>
            ) : (
...
export default function FixtureView({ phase, tournamentCars, maxQualifiers }: {
  ...
  return (
    <div className="space-y-6">
      {phase === "eliminatorias" && (
        <div className="bg-gold/10 border border-gold/20 rounded-2xl px-4 py-3 text-center">
          <p className="text-sm text-cream font-medium">El fixture se completa cuando cierren las clasificaciones.</p>
